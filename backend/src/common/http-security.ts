...
export function assertProductionOrigins(origins: string[], nodeEnv = process.env.NODE_ENV ?? 'development'): void {
...
export function assertProductionSecrets(
    ...
    if (!authSecret || authSecret === 'local-development-secret-change-me' || authSecret.length < 32) {
...
export function resolveFrontendOrigin(
    ...
    const origin = (configuredOrigin ?? (nodeEnv === 'production' ? '' : 'http://localhost:5173')).trim();
...
export function rateLimit(options: RateLimitOptions) {
    ...
    const keyFor = options.key ?? ((request: RateLimitRequest) => {
    ...
    return (request: RateLimitRequest, response: RateLimitResponse, next: () => void): void => {
        ...
        if (bucket.count > options.max) {
            ...
            response.setHeader('Retry-After', String(Math.max(1, Math.ceil((bucket.resetAt - current) / 1000))));
            response.setHeader('Content-Type', 'application/json');
