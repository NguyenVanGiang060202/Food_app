  ...
  async isReady(): Promise<boolean> {
    try {
      const result = await this.pool.query(
        `SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'restaurant') AS has_restaurant, EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'postgis') AS has_postgis, EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector') AS has_vector`,
      );
      const row = result.rows[0];
      return Boolean(row?.has_restaurant && row?.has_postgis && row?.has_vector);
    } catch {
      return false;
    }
  }
