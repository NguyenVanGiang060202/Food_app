...
interface AuthRequest { headers: { authorization?: string; cookie?: string }; user?: unknown; }
...
function readSessionCookie(header: string | undefined): string | undefined {
    ...
    try { return decodeURIComponent(token); } catch { return undefined; }
}
...
export class AuthGuard implements CanActivate {
    ...
    canActivate(context: ExecutionContext) {
        ...
        const token = request.headers.authorization?.replace(/^Bearer\s+/i, '') || readSessionCookie(request.headers.cookie);
