...
function setSessionCookie(response: HttpResponse, token: string): void {
  ...
  response.setHeader(
    'Set-Cookie',
    `${SESSION_COOKIE}=${encodeURIComponent(token)}; HttpOnly; Path=/; SameSite=Lax${secure ? '; Secure' : ''}`,
  );
}
...
function clearSessionCookie(response: HttpResponse): void {
  response.setHeader(
    'Set-Cookie',
    `${SESSION_COOKIE}=; HttpOnly; Path=/; Max-Age=0; SameSite=Lax${(process.env.NODE_ENV ?? 'development') === 'production' ? '; Secure' : ''}`,
  );
}
...
export class AuthController {
  ...
  @Post('signup') signup(@Body() body: SignUpDto) {
    return this.auth.signUp(body.email, body.password, body.confirmPassword, body.displayName);
  }
  ...
  @Post('request-password-reset') requestPasswordReset(@Body() body: RequestPasswordResetDto) {
  ...
  @Get('google') google() {
    return this.auth.googleAuthorizationUrl();
  }
  @Get('google/callback') async googleCallback(
    ...
    try {
      ...
      return response.redirect(`${origin}/auth?oauthError=google`);
    }
  }
  @Get('me') @UseGuards(AuthGuard) me(@Req() request: AuthenticatedRequest) {
    return { user: request.user };
  }
  @Get('verify-email') verifyEmail(@Query('token') token?: string) {
...
export class UsersController {
  constructor(private readonly auth: AuthService) {}
  @Get('me/preferences') @UseGuards(AuthGuard) getPreferences(
    @Req() request: AuthenticatedRequest,
  ) {
    return this.auth.getPreferences(request.user!.id).then((data) => ({ data }));
  }
  @Patch('me/preferences') @UseGuards(AuthGuard) updatePreferences(
    @Req() request: AuthenticatedRequest,
    @Body() body: UpdatePreferencesDto,
  ) {
    return this.auth.updatePreferences(request.user!.id, body).then((data) => ({ data }));
  }
}
