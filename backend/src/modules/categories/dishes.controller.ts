...
export class DishesController {
  ...
  async list(
    ...
    @Query('priceLevel') rawPriceLevel?: string,
    @Query('openNow') rawOpenNow?: string,
  ) {
    ...
    try {
      ...
      if (normalizedQuery)
        clauses.push(
          `(d.name ILIKE $${values.length} OR d.normalized_name ILIKE $${values.length} OR unaccent(d.name) ILIKE unaccent($${values.length}) OR unaccent(d.normalized_name) ILIKE unaccent($${values.length}) OR COALESCE(d.description, '') ILIKE $${values.length} OR r.name ILIKE $${values.length})`,
        );
      if (normalizedCategory) {
        values.push(normalizedCategory);
        clauses.push(
          `EXISTS (SELECT 1 FROM restaurant_category rc_filter JOIN category c_filter ON c_filter.id = rc_filter.category_id WHERE rc_filter.restaurant_id = r.id AND c_filter.slug = $${values.length} AND c_filter.is_active = true)`,
        );
      }
      if (
        ...
        clauses.push(`r.price_level = $${values.length}`);
      }
      if (openNow)
        clauses.push(
      ...
      const result = await this.database.query<DishRow>(
        `SELECT d.id, d.name, d.description, d.price_amount, d.currency_code, d.is_popular, r.id AS restaurant_id, r.name AS restaurant_name, cover.url AS cover_image_url, category.name AS category_name
                ...
                WHERE d.status = 'available' ${clauses.length ? `AND ${clauses.join(' AND ')}` : ''} ORDER BY d.is_popular DESC, r.rating DESC NULLS LAST, d.name LIMIT $1`,
        values,
      );
      return {
