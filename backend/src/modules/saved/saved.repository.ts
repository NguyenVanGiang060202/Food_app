...
const decodeCursor = (cursor?: string): SavedCursor | undefined => {
  if (!cursor) return undefined;
  try {
    const parsed = JSON.parse(Buffer.from(cursor, 'base64url').toString('utf8')) as SavedCursor;
    return parsed.createdAt && parsed.restaurantId ? parsed : undefined;
  } catch {
    return undefined;
  }
};
const encodeCursor = (row: SavedRow): string =>
  Buffer.from(JSON.stringify({ createdAt: row.created_at, restaurantId: row.id })).toString(
    'base64url',
  );
...
export class SavedRepository {
  ...
  async list(
    userId: string,
    query: SavedListOptions = { limit: 12 },
  ): Promise<{
  ...
  async save(userId: string, restaurantId: string) {
    const restaurant = await this.database.query(
      "SELECT 1 FROM restaurant WHERE id = $1 AND status = 'active'",
      [restaurantId],
    );
    if (!restaurant.rowCount) throw new NotFoundException('Restaurant was not found.');
    await this.database.query(
      'INSERT INTO saved_restaurant (user_id, restaurant_id) VALUES ($1, $2) ON CONFLICT DO NOTHING',
      [userId, restaurantId],
    );
  }
  async remove(userId: string, restaurantId: string) {
    await this.database.query(
      'DELETE FROM saved_restaurant WHERE user_id = $1 AND restaurant_id = $2',
      [userId, restaurantId],
    );
  }
  async has(userId: string, restaurantId: string) {
    const result = await this.database.query(
      'SELECT 1 FROM saved_restaurant WHERE user_id = $1 AND restaurant_id = $2',
      [userId, restaurantId],
    );
