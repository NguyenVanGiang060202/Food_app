...
export class AdminController {
  ...
  async dataSources() {
    const result = await this.database.query(
      `SELECT ds.id, ds.code, ds.name, ds.base_url AS \"baseUrl\", ds.is_active AS \"isActive\", COUNT(DISTINCT rs.id)::int AS \"restaurantSourceCount\", MAX(cr.finished_at) AS \"lastFinishedAt\" FROM data_source ds LEFT JOIN restaurant_source rs ON rs.data_source_id = ds.id LEFT JOIN crawl_run cr ON cr.data_source_id = ds.id AND cr.status = 'completed' GROUP BY ds.id ORDER BY ds.name`,
    );
  ...
  async crawlRuns(@Query() query: AdminListQueryDto) {
    ...
    const conditions = query.status ? 'WHERE cr.status = $2' : '';
    ...
    return { data: result.rows, meta: { limit: query.limit } };
  }
  @Get('processing-records')
  async processingRecords(@Query() query: AdminListQueryDto) {
    const values: unknown[] = [query.limit];
    const conditions = query.processingStatus ? 'WHERE pr.status = $2' : '';
    if (query.processingStatus) values.push(query.processingStatus);
    const result = await this.database.query(
      `SELECT pr.id, pr.crawl_run_id AS \"crawlRunId\", pr.external_id AS \"externalId\", pr.stage, pr.status, pr.attempt_count AS \"attemptCount\", pr.error_details AS \"errorDetails\", pr.processed_at AS \"processedAt\" FROM processing_record pr ${conditions} ORDER BY pr.processed_at DESC NULLS LAST, pr.id DESC LIMIT $1`,
      values,
    );
    return { data: result.rows, meta: { limit: query.limit } };
  }
  ...
  async unresolved(@Query() query: AdminListQueryDto) {
    const result = await this.database.query(
      `SELECT pr.id AS \"processingRecordId\", pr.external_id AS \"externalId\", pr.crawl_run_id AS \"crawlRunId\", pr.stage, pr.status, pr.error_details AS \"errorDetails\" FROM processing_record pr WHERE pr.stage = 'matching' AND pr.status IN ('failed', 'pending', 'processing') ORDER BY pr.id DESC LIMIT $1`,
      [query.limit],
    );
    return { data: result.rows, meta: { limit: query.limit } };
  }
  ...
  async createCrawlRun(@Body() body: CreateCrawlRunDto) {
    this.validateTarget(body.target);
    const result = await this.database.query<{ id: string }>(
      `INSERT INTO crawl_run (data_source_id, job_type, target, status) SELECT id, $2, $3::jsonb, 'queued' FROM data_source WHERE code = $1 AND is_active = true RETURNING id`,
      [body.providerCode, body.jobType, JSON.stringify(body.target)],
    );
    if (!result.rows[0]) throw new ConflictException('Provider is not active or does not exist.');
    return {
      data: {
        id: result.rows[0].id,
  ...
  async retryProcessing(@Param() params: ProcessingRecordParamDto) {
    ...
    if (!result.rows[0])
      throw new ConflictException(
        'Processing record is not eligible for retry or its provider is inactive.',
      );
    return {
      data: {
        id: result.rows[0].id,
        status: 'queued',
        retryRunId: result.rows[0].retry_run_id,
        attemptCount: result.rows[0].attempt_count,
      },
    };
  }
  private validateTarget(target: Record<string, unknown>) {
    const allowed = new Set(['city', 'district', 'category', 'query', 'location', 'limit']);
    for (const key of Object.keys(target))
      if (!allowed.has(key))
        throw new BadRequestException(`Unsupported crawl target field: ${key}.`);
    for (const key of ['city', 'district', 'category', 'query', 'location'])
      if (
        target[key] !== undefined &&
        (typeof target[key] !== 'string' || target[key].length > 200)
      )
        throw new BadRequestException(`${key} must be a string of at most 200 characters.`);
    if (
