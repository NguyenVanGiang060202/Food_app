...
test('admin key guard accepts the configured key', () => {
  ...
  assert.equal(
    new AdminApiKeyGuard().canActivate(context({ 'x-admin-api-key': 'test-admin-key' })),
    true,
  );
});
...
test('admin key guard rejects a missing or invalid key', () => {
  ...
  assert.throws(
    () => guard.canActivate(context({ 'x-admin-api-key': 'wrong-key' })),
    ForbiddenException,
  );
});
...
test('admin key guard accepts the first value from a repeated header', () => {
  process.env.ADMIN_API_KEY = 'test-admin-key';
  assert.equal(
    new AdminApiKeyGuard().canActivate(context({ 'x-admin-api-key': ['test-admin-key', 'other'] })),
    true,
  );
});
