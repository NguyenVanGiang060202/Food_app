...
test('parseAllowedOrigins trims and removes empty values', () => {
  assert.deepEqual(parseAllowedOrigins(' https://app.example , ,https://admin.example '), [
    'https://app.example',
    'https://admin.example',
  ]);
});
...
test('production rejects local CORS origins', () => {
  assert.throws(
    () => assertProductionOrigins(['http://localhost:5173'], 'production'),
...
test('production rejects missing, default, or weak secrets', () => {
  ...
  assert.throws(
    () =>
      assertProductionSecrets(
        { authSecret: 'local-development-secret-change-me', adminApiKey: 'a'.repeat(32) },
        'production',
      ),
    /strong AUTH_SECRET/,
  );
  assert.throws(
    () =>
      assertProductionSecrets({ authSecret: 'a'.repeat(32), adminApiKey: 'short' }, 'production'),
    /strong ADMIN_API_KEY/,
  );
  assert.doesNotThrow(() =>
    assertProductionSecrets(
      { authSecret: 'a'.repeat(32), adminApiKey: 'b'.repeat(32) },
      'production',
    ),
  );
});
...
test('frontend OAuth origin must be explicit and allowlisted in production', () => {
  assert.equal(
    resolveFrontendOrigin(undefined, ['http://localhost:5173']),
    'http://localhost:5173',
  );
  assert.equal(
    resolveFrontendOrigin('https://app.example/', ['https://app.example'], 'production'),
    'https://app.example',
  );
  assert.throws(
    () => resolveFrontendOrigin(undefined, ['https://app.example'], 'production'),
    /explicit APP_ORIGIN/,
  );
  assert.throws(
    () => resolveFrontendOrigin('https://other.example', ['https://app.example'], 'production'),
    /CORS origins/,
  );
  assert.throws(
    () => resolveFrontendOrigin('https://app.example/auth', ['https://app.example'], 'production'),
    /HTTP\(S\) origin/,
  );
});
...
test('request size middleware rejects oversized content length', () => {
  ...
  requestSizeLimit(100)({ headers: { 'content-length': '101' } }, response, () => {
    called = true;
  });
