...
test('retryProcessing enqueues a discovery crawl run and returns its id', async () => {
    const { controller, calls } = createController([{ id: 'record-1', attempt_count: 2, retry_run_id: 'run-2' }]);
...
test('createCrawlRun rejects an unbounded target field', async () => {
    ...
    await assert.rejects(
        ...
        (error: unknown) => error instanceof BadRequestException && /Unsupported crawl target field: cookies/.test(error.message),
    );
