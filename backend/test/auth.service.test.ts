...
test('signUp does not issue a session before email verification', async () => {
  ...
  const database = {
    query: async <T>(sql: string): Promise<{ rows: T[]; rowCount: number }> => {
      ...
      return {
        rows: [{ id: 'user-1', email: 'user@example.com', displayName: 'User' } as T],
        rowCount: 1,
      };
    },
  };
  ...
  const database = {
    query: async <T>(sql: string): Promise<{ rows: T[]; rowCount: number }> =>
      sql.startsWith('SELECT 1')
        ? { rows: [], rowCount: 0 }
        : {
            rows: [{ id: 'user-2', email: 'user@example.com', displayName: null } as T],
            rowCount: 1,
          },
  };
...
test('verifyToken converts malformed payloads into 401 errors', () => {
  ...
  assert.throws(
    ...
    (error: unknown) =>
      error instanceof UnauthorizedException && /không hợp lệ/.test(error.message),
  );
});
...
test('verifyToken returns the identity encoded in the signed token', () => {
  ...
  try {
    ...
    assert.deepEqual(service.verifyToken(`${encoded}.${signature}`), {
      id: 'user-2',
      email: 'other@example.com',
      displayName: 'Other',
    });
...
test('updatePreferences persists only supplied preference fields', async () => {
  ...
  const database = {
    query: async <T>(
      ...
      return {
        rows: [
          {
            favorite_category_slugs: ['bun'],
            dietary_preferences: [],
            preferred_price_levels: [3],
          } as T,
        ],
        rowCount: 1,
      };
  ...
  const result = await auth.updatePreferences('user-1', {
    favoriteCategorySlugs: ['bun'],
    preferredPriceLevels: [3],
  });
