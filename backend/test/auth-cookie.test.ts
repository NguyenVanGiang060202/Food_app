...
test('auth guard accepts the signed session cookie', () => {
  const auth = {
    verifyToken: (token: string) => ({ id: token, email: 'user@example.com', displayName: null }),
  };
...
test('auth guard prefers the bearer header over the session cookie', () => {
  ...
  const auth = {
    verifyToken: (token: string) => {
      tokens.push(token);
      return { id: token, email: 'user@example.com', displayName: null };
    },
  };
  const guard = new AuthGuard(auth as never);
  const request = {
    headers: {
      ...
      cookie: 'food-discovery-session=cookie-token',
    },
  };
  const context = {
    switchToHttp: () => ({ getRequest: () => request }),
