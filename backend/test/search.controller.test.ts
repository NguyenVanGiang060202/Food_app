...
import type { RestaurantPage, RestaurantSummary } from '../src/modules/restaurants/restaurants.types';
...
const page = (data: RestaurantSummary[]): RestaurantPage => ({ data, meta: { nextCursor: null, limit: 20 } });
...
test('for-you uses all saved category and price preferences and removes duplicates', async () => {
    ...
    const databaseService = {
        list: async (filters: Record<string, unknown>) => {
            ...
            return page([restaurant(`${category ?? 'none'}-${priceLevel ?? 'none'}`), restaurant('shared')]);
        },
    };
    const auth = { getPreferences: async () => ({ favoriteCategorySlugs: ['pho', 'bun'], dietaryPreferences: ['vegetarian'], preferredPriceLevels: [1, 2] }) };
    const controller = new RecommendationsController(databaseService as never, auth as never);

    const result = await controller.forYou({ user: { id: 'user-1', email: 'user@example.com', displayName: null } });
...
test('for-you falls back to rating when saved preferences have no matches', async () => {
    ...
    const auth = { getPreferences: async () => ({ favoriteCategorySlugs: ['pho'], dietaryPreferences: [], preferredPriceLevels: [] }) };
    const controller = new RecommendationsController(databaseService as never, auth as never);

    const result = await controller.forYou({ user: { id: 'user-2', email: 'user@example.com', displayName: null } });
