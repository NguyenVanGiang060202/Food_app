...
import { ChevronDown, LocateFixed, RotateCcw, Send, SlidersHorizontal, X } from "lucide-react";
...
import { getRecommendations, interpretSearch, listDishes, toRestaurant, type BackendDish, type InterpretedSearch, type RecommendationItem } from "@/lib/api";
...
const suggestions = ["Trời lạnh, xin một tô ấm bụng", "Ăn nhẹ buổi chiều, ví còn 50k", "Cay cho tỉnh người, đừng cay đời", "Bữa trưa chắc bụng gần đây"];
const quickFilters = [
...
export function AskPage() {
  ...
  const [turns, setTurns] = useState<AskTurn[]>(() => {
    try { return JSON.parse(sessionStorage.getItem("bep:ask-session") ?? "[]") as AskTurn[]; } catch { return []; }
  });
  ...
  const activeCount = filters.attrs.length + (filters.maxDistanceKm ? 1 : 0) + (filters.openNow ? 1 : 0) + (filters.minRating ? 1 : 0) + (filters.priceLevel ? 1 : 0);
    ...
    try {
      ...
      const recommendations = await getRecommendations({
        ...
        filters: {
          ...
          ...(requestLocation && nextFilters.maxDistanceKm ? { radiusMeters: nextFilters.maxDistanceKm * 1000 } : {}),
        },
      }, controller.signal);
      const dishes = await listDishes(12, trimmed, { openNow: nextFilters.openNow || undefined }).catch(() => []);
      ...
      setTurns((current) => [...current, { query: trimmed, answer: recommendations.length ? `Bếp tìm được ${recommendations.length} quán hợp cơn thèm này.` : "Bếp chưa thấy quán hợp gu. Nới bộ lọc một chút nhé?" }]);
    } catch (cause) {
      if (!controller.signal.aborted) setTurns((current) => [...current, { query: trimmed, answer: cause instanceof Error ? cause.message : "Bếp bị khét một nhịp. Thử lại nhé." }]);
    } finally {
  ...
  const refineSearch = (nextFilters: Filters) => { setSearchContext((current) => ({ ...current, filters: nextFilters })); if (searchContext.keyword) void runSearch(searchContext.keyword, nextFilters); };
  const reset = () => { requestRef.current?.abort(); setQuery(""); setResult([]); setDishCatalog([]); setInterpreted(null); setTurns([]); setPendingQuery(null); setActive(null); setSearchContext({ keyword: "", filters: { attrs: [], openNow: false }, sort: "relevance" }); sessionStorage.removeItem("bep:ask-session"); textareaRef.current?.focus(); };
  ...
  const chatPane = <div className="flex min-h-0 flex-1 flex-col">
    {!submitted ? <div className="flex flex-1 flex-col justify-center py-10">
      <h1 className="mt-5 max-w-lg text-balance font-display text-4xl leading-[1.05] md:text-6xl">Bụng đang <em className="italic text-primary">réo</em> món gì?</h1>
      <p className="mt-4 max-w-md text-muted-foreground">Kể Bếp nghe một cảm giác: nóng, cay, nhẹ bụng, ví mỏng hay gần đây. Bếp sẽ dọn ra món, quán và lý do để bạn chốt kèo.</p>
      <div className="mt-8 text-[11px] uppercase tracking-widest text-muted-foreground">Mồi lửa cơn thèm</div>
    ...
    </div> : <div className="flex-1 space-y-5 overflow-auto py-6">
      {turns.map((turn, index) => <div key={`${turn.query}-${index}`} className="space-y-2 border-t border-border/60 pt-4"><div className="ml-auto max-w-[90%] rounded-2xl bg-primary/10 px-4 py-3 text-sm">{turn.query}</div><div className="text-sm leading-6">{turn.answer}</div>{index === turns.length - 1 && <InterpretationChips value={interpreted} />}</div>)}
      {busy && pendingQuery && <div className="space-y-3 border-t border-border/60 pt-4"><div className="ml-auto max-w-[90%] rounded-2xl bg-primary/10 px-4 py-3 text-sm">{pendingQuery}</div><div className="flex items-center justify-between gap-3 text-sm text-muted-foreground"><span>Bếp đang đảo chảo dữ liệu...</span><button onClick={() => requestRef.current?.abort()} className="rounded-full border border-border px-3 py-1 text-xs hover:border-primary hover:text-primary">Dừng bếp</button></div></div>}
    </div>}
    <div className="sticky bottom-16 z-30 space-y-3 bg-background pb-3 pt-2 md:bottom-0 md:pb-4">
      {showFilters && <FilterPanel filters={filters} toggleAttr={toggleAttr} onChange={refineSearch} hasLocation={location !== null} onLocate={locate} />}
      <div className="flex flex-wrap items-center gap-2">
        ...
        {filters.attrs.map((attr) => <FilterChip key={attr} label={attrLabel(attr)} onRemove={() => toggleAttr(attr)} />)}
        ...
        {submitted && <button type="button" onClick={reset} className="ml-auto inline-flex items-center gap-1.5 text-xs text-muted-foreground"><RotateCcw className="h-3.5 w-3.5" />Hỏi mẻ mới</button>}
      </div>
      <div className="rounded-2xl border border-border bg-card p-3 shadow-lift"><textarea ref={textareaRef} value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void send(); } }} className="min-h-20 w-full resize-none bg-transparent text-sm outline-none" placeholder="Bụng đang réo gì? Kể Bếp nghe..." /><div className="flex items-center justify-between pt-2 text-[11px] text-muted-foreground"><span>Enter để hỏi Bếp · Shift + Enter để xuống dòng</span><button aria-label="Hỏi Bếp" onClick={() => void send()} disabled={busy || !query.trim()} className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-primary text-white transition-transform hover:scale-105 disabled:cursor-not-allowed disabled:opacity-50">{busy ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white/40 border-t-white" /> : <Send className="h-4 w-4" />}</button></div></div>
    </div>
  </div>;
  ...
  return <div className="flex min-h-screen flex-col"><Header /><main className="w-full flex-1 px-4 pb-24 pt-4 md:px-6 md:pb-4"><div className="grid gap-4 lg:h-[calc(100vh-6rem)] lg:grid-cols-[minmax(0,22rem)_minmax(0,1fr)_minmax(0,22rem)]"><aside className="order-2 min-h-0 overflow-y-auto lg:order-1"><SearchContextChips context={searchContext} onRemove={refineSearch} /><div className="mb-2 mt-4 text-[11px] uppercase tracking-widest text-muted-foreground">Quán hợp bụng ({resultRestaurants.length})</div>{resultRestaurants.length ? <Stagger className="space-y-2">{resultRestaurants.map((restaurant, index) => <StaggerItem key={restaurant.id} onMouseEnter={() => setActive(restaurant.id)}><RestaurantResult id={restaurant.id} restaurant={restaurant} explanation={result[index]?.explanation} matchingNames={matchingDishNames.get(restaurant.id)} onSelect={() => setActive(restaurant.id)} compact /></StaggerItem>)}</Stagger> : <div className="rounded-2xl border border-dashed border-border p-5 text-sm text-muted-foreground"><p className="font-medium text-foreground">Bếp chưa thấy quán hợp gu.</p><p className="mt-2">Nới bán kính, bớt một yêu cầu khó tính hoặc hỏi Bếp theo cách khác nhé.</p></div>}<div className="mb-2 mt-5 text-[11px] uppercase tracking-widest text-muted-foreground">Món hợp gu</div><Stagger className="space-y-2">{resultDishes.map((dish) => <StaggerItem key={dish.id}><DishResult id={dish.id} dish={dish} /></StaggerItem>)}</Stagger></aside><div className="relative order-3 h-[280px] overflow-hidden rounded-2xl border border-border bg-parchment lg:order-2 lg:h-full"><MapCanvas restaurants={resultRestaurants} activeId={active} onHover={setActive} userLocation={location ? [location.latitude, location.longitude] : null} /></div><section className="order-1 flex min-h-0 flex-col rounded-2xl border border-border bg-card/40 p-3 lg:order-3">{chatPane}</section></div></main></div>;
}
...
function SearchContextChips({ context, onRemove }: { context: SearchContext; onRemove: (filters: Filters) => void }) {
  ...
  return <div className="rounded-2xl border border-primary/15 bg-primary/5 p-3"><div className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-primary">Bếp đang hiểu là</div><div className="flex flex-wrap gap-1.5">{chips.map((chip) => <button key={chip.label} type="button" onClick={chip.remove} className="inline-flex items-center gap-1 rounded-full border border-primary/20 bg-background px-2.5 py-1.5 text-xs text-primary hover:border-primary">{chip.label}<X className="h-3 w-3" /></button>)}<span className="self-center text-[11px] text-muted-foreground">Ưu tiên: hợp câu hỏi</span></div></div>;
}

function InterpretationChips({ value }: { value: InterpretedSearch | null }) {
  if (!value) return null;
  const chips = [value.filters.category ? `Nhóm: ${value.filters.category}` : null, value.filters.district ? `Khu vực: ${value.filters.district}` : null, ...value.filters.attributes.map((attribute) => `Mục đích: ${attribute}`)].filter(Boolean) as string[];
...
function FilterPanel({ filters, toggleAttr, onChange, hasLocation, onLocate }: { filters: Filters; toggleAttr: (value: string) => void; onChange: (next: Filters) => void; hasLocation: boolean; onLocate: () => void }) {
