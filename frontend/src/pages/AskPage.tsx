    ...
    <div className="flex min-h-0 flex-1 flex-col">
      {!submitted ? (
        <div className="flex flex-1 flex-col justify-center py-10">
          ...
          <div className="mt-3 flex flex-wrap gap-2">{suggestions.map((item) => <button key={item} onClick={() => void send(item)} className="rounded-full border border-border bg-card px-4 py-2 text-sm text-foreground/80 hover:-translate-y-0.5 hover:border-primary/50 hover:text-primary">{item}</button>)}</div>
        </div>
      ) : (
    ...
    <main className="w-full flex-1 px-4 pb-24 pt-4 md:px-6 md:pb-4">
      <div className="grid gap-4 lg:h-[calc(100vh-6rem)] lg:grid-cols-[minmax(0,22rem)_minmax(0,1fr)_minmax(0,22rem)]">
        <aside className="order-2 min-h-0 overflow-y-auto lg:order-1">
          ...
          {resultRestaurants.length ? <div className="space-y-2">{resultRestaurants.map((restaurant, index) => <div key={restaurant.id} onMouseEnter={() => setActive(restaurant.id)}><RestaurantResult id={restaurant.id} restaurant={restaurant} explanation={result[index]?.explanation} matchingNames={matchingDishNames.get(restaurant.id)} onSelect={() => setActive(restaurant.id)} compact /></div>)}</div> : <div className="rounded-2xl border border-dashed border-border p-5 text-sm text-muted-foreground"><p className="font-medium text-foreground">Không có quán nào khớp bộ lọc hiện tại.</p><p className="mt-2">Thử mở rộng bán kính, bỏ bớt một bộ lọc hoặc xem lại tất cả kết quả phù hợp với từ khóa.</p><button type="button" onClick={() => refineSearch({ ...filters, maxDistanceKm: undefined, minRating: undefined, priceLevel: undefined, openNow: false })} className="mt-3 rounded-full bg-foreground px-3 py-1.5 text-xs text-background">Bỏ bộ lọc giới hạn</button></div>}
          <div className="mb-2 mt-5 text-[11px] uppercase tracking-widest text-muted-foreground">Món hợp gu</div>
          <div className="space-y-2">{resultDishes.map((dish) => <DishResult key={dish.id} id={dish.id} dish={dish} />)}</div>
        </aside>
