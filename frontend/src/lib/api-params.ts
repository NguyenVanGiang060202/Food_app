...
export function buildRestaurantQuery(options: RestaurantQueryOptions = {}): string {
  ...
  if (options.radiusMeters !== undefined && options.latitude !== undefined && options.longitude !== undefined) {
...
export function buildSearchQuery(query: string, options: SearchQueryOptions = {}): string {
  ...
  if (options.latitude !== undefined && options.longitude !== undefined) {
    params.set("latitude", String(options.latitude));
    params.set("longitude", String(options.longitude));
    if (options.radiusMeters !== undefined) params.set("radiusMeters", String(options.radiusMeters));
  }
  return params.toString();
}

export function buildDishQuery(limit = 20, query?: string, options: { category?: string; openNow?: boolean } = {}): string {
