...
import { clearSearchFilters, readOptionalNumber, updateSearchParam } from "../src/lib/search-url.ts";

test("updateSearchParam preserves existing filters and encodes new values", () => {
  assert.equal(updateSearchParam("q=pho&sort=rating", "category", "noodles"), "/search?q=pho&sort=rating&category=noodles");
});

test("updateSearchParam removes a filter without dropping the query", () => {
  assert.equal(updateSearchParam("q=pho&open=true&sort=rating", "open"), "/search?q=pho&sort=rating");
});
