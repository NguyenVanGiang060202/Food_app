...
test("restaurant query keeps bounded filters and repeated tastes", () => {
  const params = new URLSearchParams(buildRestaurantQuery({
    limit: 12,
    category: "pho bo",
    latitude: 10.77,
    longitude: 106.7,
...
test("search query only sends radius when both coordinates are present", () => {
  const missingLocation = new URLSearchParams(buildSearchQuery("  cà phê  ", { radiusMeters: 1000, latitude: 10.7 }));
  assert.equal(missingLocation.get("query"), "cà phê");
  assert.equal(missingLocation.get("radiusMeters"), null);

  const withLocation = new URLSearchParams(buildSearchQuery("  cà phê  ", { radiusMeters: 1000, latitude: 10.7, longitude: 106.6 }));
...
test("dish query omits empty optional filters", () => {
  ...
  const params = new URLSearchParams(buildDishQuery(10, "bún bò", { category: "noodles", openNow: true }));
