...
export interface CrawlRunPipeline {
    ...
    recordFailure(crawlRunId: string, externalId: string | undefined, stage: 'validation' | 'normalization', error: unknown): Promise<void>;
...
export class DiscoveryCrawlExecutor implements CrawlRunExecutor {
    ...
    async execute(job: CrawlRunJob, provider: DataProviderAdapter): Promise<void> {
        if (job.jobType !== 'discovery') {
            throw new Error(`Unsupported crawl job type '${job.jobType}'. Discovery executor accepts only 'discovery'.`);
        }
...
function toDiscoveryInput(target: Record<string, unknown>): DiscoveryInput {
    const limit = typeof target.limit === 'number' && Number.isFinite(target.limit)
