...
export function validateSourceRecord(record: SourceRestaurantRecord): void {
  ...
  if (!record.collectedAt || Number.isNaN(Date.parse(record.collectedAt)))
    throw new Error('collectedAt must be a valid ISO timestamp.');
  if (
    record.coordinates &&
    ...
    throw new Error('coordinates are outside valid geographic ranges.');
  ...
  if (
    record.priceLevel !== undefined &&
    (!Number.isInteger(record.priceLevel) || record.priceLevel < 1 || record.priceLevel > 4)
  )
    throw new Error('priceLevel must be between 1 and 4.');
  for (const image of record.images ?? []) {
    if (!/^https?:\/\//i.test(image.url)) throw new Error('image URLs must use http or https.');
  }
}
