...
export function buildPlaywrightQueryPlan(
  options: PlaywrightQueryPlanOptions,
): PlaywrightQueryTarget[] {
  ...
  const locations = districts.length > 0 ? districts.map((d) => `${d}, ${city}`) : [city];
