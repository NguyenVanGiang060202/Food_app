...
const DISTRICTS =
  process.env.CRAWL_DISTRICTS?.split(',')
    .map((s) => s.trim())
    .filter(Boolean) ?? [];
...
const QUERIES = process.env.CRAWL_QUERIES?.split(',')
...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const [index, target] of plan.entries()) {
      const jobTarget: Record<string, unknown> = {
        query: target.query,
    ...
    console.error(
      JSON.stringify({ event: 'batch_crawl_complete', completed, failed, total: plan.length }),
    );
  } finally {
