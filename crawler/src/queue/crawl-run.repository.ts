...
export class CrawlRunRepository {
  ...
  async claimNext(): Promise<CrawlRunJob | undefined> {
    ...
    try {
      ...
      await client.query(
        `UPDATE crawl_run SET status = 'running', started_at = COALESCE(started_at, now()), worker_id = $2, lease_expires_at = now() + ($3 * interval '1 second'), error_message = NULL WHERE id = $1`,
        [job.id, this.workerId, this.leaseSeconds],
      );
      await client.query('COMMIT');
      return job;
    } catch (error) {
  ...
  async markFailed(id: string, error: unknown): Promise<void> {
    ...
    await this.pool.query(
      `UPDATE crawl_run SET status = 'failed', finished_at = now(), worker_id = NULL, lease_expires_at = NULL, error_message = $2 WHERE id = $1 AND status = 'running' AND (worker_id = $3 OR lease_expires_at < now())`,
      [id, message, this.workerId],
    );
  }

  async renewLease(id: string): Promise<void> {
    await this.pool.query(
      `UPDATE crawl_run SET lease_expires_at = now() + ($2 * interval '1 second') WHERE id = $1 AND status = 'running' AND worker_id = $3`,
      [id, this.leaseSeconds, this.workerId],
    );
  }
}
