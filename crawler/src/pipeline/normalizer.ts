...
export interface NormalizedRestaurantRecord extends SourceRestaurantRecord {
...
export function normalizeSourceRecord(record: SourceRestaurantRecord): NormalizedRestaurantRecord {
  ...
  return {
    ...
    formattedAddress:
      [clean(record.address), clean(record.district), clean(record.city)]
        .filter(Boolean)
        .join(', ') || name,
    categories: [
      ...new Set(
        (record.categories ?? []).map((value) => value.trim().toLowerCase()).filter(Boolean),
      ),
    ],
  };
}
