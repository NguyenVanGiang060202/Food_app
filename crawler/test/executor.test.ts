...
import {
  ...
  type CrawlRunPipeline,
} from '../src/jobs/discovery-crawl.executor';
...
const job: CrawlRunJob = {
  ...
  jobType: 'discovery',
  target: { city: 'HCMC', limit: 2.9 },
};
const validRecord: SourceRestaurantRecord = {
  providerCode: 'provider',
...
test('discovery executor processes valid records and completes with counters', async () => {
  ...
  const pipeline: CrawlRunPipeline = {
    async process(runId, record) {
      processed.push(`${runId}:${record.normalizedName}`);
      return 'restaurant-1';
    },
    async recordFailure(...args) {
      failures.push(args);
    },
    async finishCrawlRun(...args) {
      completed = args;
    },
  };
