...
test('fixture provider respects bounded filters', async () => {
  const names: string[] = [];
  for await (const record of new FixtureProvider().discover({ district: 'District 1', limit: 1 })) names.push(record.name);
  assert.equal(names.length, 1);
});

test('normalizer creates a stable accent-free comparison name', () => {
  const normalized = normalizeSourceRecord({ providerCode: 'google_maps_playwright', externalId: 'x', collectedAt: new Date().toISOString(), name: '  Bếp   Nhà Mình  ', sourceMetadata: {} });
...
test('source validation rejects invalid coordinates', () => {
  assert.throws(() => validateSourceRecord({ providerCode: 'google_maps_playwright', externalId: 'x', collectedAt: new Date().toISOString(), name: 'Test', coordinates: { latitude: 100, longitude: 0 }, sourceMetadata: {} }), /coordinates/);
});