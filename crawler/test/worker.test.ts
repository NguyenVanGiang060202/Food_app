...
function store(
  next: CrawlRunJob | undefined,
): CrawlRunStore & { failed: Array<{ id: string; error: unknown }> } {
...
test('worker returns false when no queued crawl run exists', async () => {
  ...
  const worker = new CrawlRunWorker(repository, new Map([[provider.providerCode, provider]]), {
    async execute() {
...
test('worker fails a run when its provider is not registered', async () => {
  ...
  const worker = new CrawlRunWorker(repository, new Map(), {
    async execute() {
      throw new Error('must not execute');
    },
  });
...
test('worker validates provider and records executor failures', async () => {
  ...
  let validated = false;
  const configuredProvider: DataProviderAdapter = {
    ...provider,
    async validateConfiguration() {
      validated = true;
    },
  };
  const worker = new CrawlRunWorker(
    repository,
    new Map([[provider.providerCode, configuredProvider]]),
    {
      async execute() {
...
test('worker rejects a provider registry identity mismatch', async () => {
  const repository = store(job);
  const mismatchedProvider: DataProviderAdapter = {
    ...provider,
    providerCode: 'different-provider',
  };
  const worker = new CrawlRunWorker(
    repository,
    new Map([[provider.providerCode, mismatchedProvider]]),
    {
      async execute() {
        throw new Error('must not execute');
      },
